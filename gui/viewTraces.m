...
%       end
      ...
      if(isempty(basePatternList) && isfield(experiment, 'traceUnsupervisedEventDetectionOptionsCurrent'))
        basePatternList = strsplit(sprintf('Unsupervised: %d,', 1:experiment.traceUnsupervisedEventDetectionOptionsCurrent.numberGroups+1),',');
...
%--------------------------------------------------------------------------
function menuUnsupervisedEventDetection(~, ~)
  [success, traceUnsupervisedEventDetectionOptionsCurrent] = preloadOptions(experiment, traceUnsupervisedEventDetectionOptions, gui, true, false);
  
  if(success)
    experiment.traceUnsupervisedEventDetectionOptionsCurrent = traceUnsupervisedEventDetectionOptionsCurrent;
    experiment = traceUnsupervisedEventDetection(experiment, traceUnsupervisedEventDetectionOptionsCurrent);
    if(~isempty(gui))
      setappdata(gui, 'traceUnsupervisedEventDetectionOptionsCurrent', traceUnsupervisedEventDetectionOptionsCurrent);
    end
    [patterns, basePatternList] = generatePatternList(experiment);
    updateMenu();
    updateImage(true);
